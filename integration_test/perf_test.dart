...
import 'package:perf_demo/main.dart' as app;
...
void main() {
  ...
  // app_startup 은 여기 없다. 이미 떠 있는 프로세스에서 재면 위젯 빌드 시간(수 ms)일 뿐이라
  // startup 이 아니다. 그건 `perfkit run` 이 `flutter run --trace-startup` 으로 따로 잰다.

  scenario('login', (tester) async {
    await tester.enterText(find.byKey(const Key('login_id')), 'tester');
    await tester.enterText(find.byKey(const Key('login_pw')), 'secret');
  ...
  scrollScenario('community_scroll', const Key('community_list'),
      setUp: (tester) async {
    await _login(tester);
    await _tap(tester, const Key('tab_community'));
  });

  scrollScenario('community_detail', const Key('detail_list'),
      setUp: (tester) async {
    await _login(tester);
    await _tap(tester, const Key('tab_community'));
    await _tap(tester, const Key('post_3'));
  });

  scrollScenario('image_heavy_screen', const Key('gallery_grid'),
      setUp: (tester) async {
    await _login(tester);
    await _tap(tester, const Key('tab_gallery'));
  });
}
...
void scenario(
  ...
  testWidgets(name, (tester) async {
    ...
    // setUp 은 측정 밖이다. 화면 진입과 워밍업이 여기서 끝난다.
    await setUp?.call(tester);
    await tester.pumpAndSettle();
